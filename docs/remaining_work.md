...
- Step 3 집계 분석 — 완료 (`/analytics/datasets`, `/analytics/presets`, `/analytics/query`, `/analytics/explore-query`, `/analytics/view-tables`, generated/saved views, SQL allowlist, read-only SQL hardening)
- Step 4 Docker 실행 — 완료권 (`docker-compose.yml`에 app/db/redis/event-generator/frontend 포함)
- Step 5 시각화 — 완료권 (`frontend/` Next.js Superset-style Charts/SQL Lab/View Tables, chart preview + SQL Lab table)
- 제출 README — 완료권 (`README.md`에 요구사항 충족 위치, 실행 방법, screenshot placeholder, 검증 checklist 반영)
...
- 시각화 방식 설명
- 안전한 SQL 제한 정책 요약
  - generated/saved view allowlist
  - `JOIN` / `WHERE` / `GROUP BY` / read-only CTE/subquery는 허용하되 모든 physical relation allowlist 검증
  - 함수는 좁은 allowlist만 허용하고 `pg_sleep`, `pg_advisory_lock`, `set_config`, `version`, `generate_series` 차단
  - `information_schema`, `pg_catalog` system catalog/schema 접근 차단
  - SELECT INTO / locking read 거부
  - TABLESAMPLE 거부
  - SQL text length / row limit + PostgreSQL timeout
  - accepted/rejected/completed SQL Lab audit log
- screenshot placeholder 경로
- 제출 전 검증 checklist

### 3.2 제출용 증빙 수집

아래 실제 증빙은 최종 제출 직전 최신 실행 환경에서 다시 캡처하면 됩니다. README에는 placeholder 경로를 먼저 잡아 두었습니다.
...
- SQL Lab 실행 후 table-only 표시 스크린샷
- View Tables preview/save와 Chart Builder JOIN 스크린샷
...
6. `fect/structured-explore-query`
7. View Tables / Chart JOIN 후속 브랜치

`fect/structured-explore-query`는 Superset-style UI branch 위에 쌓인 structured Explore query 브랜치입니다. View Tables / Chart JOIN 후속 작업은 이 흐름 위에서 사용자 생성 view table과 1-hop JOIN을 추가한 범위입니다.
...
- README placeholder를 실제 제출용 이미지 파일로 교체
...
- PostgreSQL 외 MySQL/DuckDB/ClickHouse adapter 구현
- OpenTelemetry exporter 연결
- metric / alert 시스템 도입
