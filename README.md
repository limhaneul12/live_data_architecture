...
POST /analytics/connection-test
...
- `/analytics/connection`은 현재 analytics DB 종류와 password-masked address를 UI에 표시하기 위한 metadata만 반환합니다.
- `/analytics/connection-test`는 Connections 화면에서 입력한 PostgreSQL 주소로 `SELECT 1` 연결 가능 여부만 확인합니다. 성공해도 SQL Lab/Explore runtime DSN을 교체하거나 저장하지 않습니다.
